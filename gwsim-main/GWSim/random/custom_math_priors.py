...
# tex document mentioned here is the icarogw latex paper from Simone https://www.overleaf.com/project/605cb4af1970e4640f4df30f
...
#            print("prob_ret: {}, {} ; {} ; self.norm:{}".format(prob_ret,self.origin_prob.log_prob(x),_np.log(window),self.norm))
        else:
...
def cm_smoothing(x,dm):

    S = np.ones(len(x))
    idx = np.where(x<np.min(x)+dm)[0]
    p = x[idx]-np.min(x)
    S[idx]= 1/(1+np.exp(np.nan_to_num(dm*(1/p+1/(p-dm)))))
    return S

def cm_powerlaw(x,power,smoothing=False,dm=0):

    xmin = np.min(x)
    xmax = np.max(x)
    if power == 1: int_power = np.log(xmax/xmin)
    else: int_power = (xmax**(1-power)-xmin**(1-power))/(1-power)
    res = (x**(-power))/int_power
    if smoothing: res *= cm_smoothing(x,dm)
    return res

def cm_powerlaw_gaussian(x,power,Lambda,mu,sigma,smoothing,dm):
    
    y_power = cm_powerlaw(x,power)
    y_gaussian = 1/(sigma*np.sqrt(2*np.pi))*np.exp(-(x-mu)**2/(2*sigma**2))
    y = ((1-Lambda)*y_power+Lambda*y_gaussian)*cm_smoothing(x,dm)
    return y

def cm_powerlaw_double_gaussian(x,power,Lambda_low,mu_low,sigma_low,Lambda,mu,sigma,smoothing,dm): # for m1

    norm = 1/(np.sqrt(2*np.pi))
    y_power = cm_powerlaw(x,power)
    y_gaussian_low = (norm/sigma_low)*np.exp(-(x-mu_low)**2/(2*sigma_low**2))
    y_gaussian = (norm/sigma)*np.exp(-(x-mu)**2/(2*sigma**2))
    y = ((1-Lambda)*y_power+Lambda*(1-Lambda_low)*y_gaussian+Lambda*Lambda_low*y_gaussian_low)*cm_smoothing(x,dm)
    return y
